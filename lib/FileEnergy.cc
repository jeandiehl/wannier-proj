...
void FileEnergy::readBands (std::vector<std::vector<double> >& energy, std::vector<double>& weight,std::vector<std::string>& kkpath, std::vector<int>& kkpathIndex) {
unsigned int Nkpoints = 0; // indexing the kpoints
double Nweight = 0.0;
	std::string line;
	...
	if (myfile.is_open()) {
		while ( myfile.good() ) {
			...
			if (line.length() > 86) {
				if (line.substr(0,1).compare(" ") == 0 && line.substr(84,3).compare("   ") == 0) {
					//std::stringstream s(line.substr(84,3));
				...
				if (line.substr(57,10).compare("          ") != 0 && line.substr(84,3).compare("   ") == 0) {
					kkpath.push_back(line.substr(57,10));
					kkpathIndex.push_back(Nkpoints-1);
				}
...
void FileEnergy::readBands (std::vector<std::vector<double> >& energy, std::vector<double>& weight) {
unsigned int Nkpoints = 0; // indexing the kpoints
double Nweight = 0.0;
	std::string line;
